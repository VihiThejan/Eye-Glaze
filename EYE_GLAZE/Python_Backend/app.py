...
def predict():
    ...
    try:
        ...
        # Use ML prediction directly with confidence-based adjustment
        if raw_prediction == 2:  # STRESS predicted
            ring_count = 2
        elif raw_prediction == 1:  # PARTIAL predicted
            # If very low confidence in PARTIAL but high in NORMAL, downgrade
            if prob_normal > 0.35 and confidence < 0.60:
                ring_count = 0
            else:
                ring_count = 1
        else:  # NORMAL predicted
            # If very low confidence in NORMAL but high in STRESS/PARTIAL, upgrade
            if prob_stress > 0.40 and confidence < 0.60:
                ring_count = 2
            elif prob_partial > 0.40 and confidence < 0.60:
                ring_count = 1
            else:
                ring_count = 0
